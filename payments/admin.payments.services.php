...
<head>
	...
	<title>Receipts per Service</title>
...
//querying the sql and calling for db connection else exit execution of query and display a connection error
    $customer_serv_receipt= mysqli_query($conn,$query) or die(mysqli_error($conn));

      //return number of rows in a result set
    $num=mysqli_num_rows($customer_serv_receipt);
    ?>

<div class="container">
	<h1>Payment filter by Service</h1> <br>

<BR> <BR> <BR>
<!-- Form to filter by services offered. Form Data sent to admin.payments.services.date.php upon clicking filter-->
<form action="admin.payments.services.date.php" method="POST" ><br><br>
    <div class=service>
...
<!--  filter by service -->
  ...
  <button type="submit" name="admin_receipts" style="width: 10%; height:5%; font-size:18px;float:right;
  margin-right:50px;">
                            Generate
                        </button> <br>
